...
assert(initilized == false);//breaking encap
...
void Poo::Update()
{
	assert(initilized == true);//verifys that it is initilized
	x += vx;
	y += vy;
...
{
	assert(initilized == true);//verifys that poo is initilized
...
{
	assert(initilized == true);//verifys that poo is initilized
	const int duderight = dude.GetX() + width;
	const int dudebottom = dude.GetY() + height;
